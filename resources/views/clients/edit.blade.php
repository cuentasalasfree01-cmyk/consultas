                ...
                <div class="card shadow-sm border-0 p-4" style="border-radius: 18px;">
                    <div class="card-body">
                        <h3 class="card-title mb-4 text-primary">
                            <i class="bi bi-person-lines-fill"></i> Editar persona
                        </h3>
                        <form action="{{ route('clients.update', $client) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="id_type" id="id_type" class="form-select" required>
                                    ...
                                    <option value="CC" {{ old('id_type', $client->id_type) == 'CC' ? 'selected' : '' }}>
                                        Cédula de ciudadanía</option>
                                    <option value="CE" {{ old('id_type', $client->id_type) == 'CE' ? 'selected' : '' }}>
                                        Cédula de extranjería</option>
                                    <option value="TI" {{ old('id_type', $client->id_type) == 'TI' ? 'selected' : '' }}>
                                        Tarjeta de identidad</option>
                                    <option value="PA" {{ old('id_type', $client->id_type) == 'PA' ? 'selected' : '' }}>
                                        Pasaporte</option>
                                    <option value="NIT"
                                        {{ old('id_type', $client->id_type) == 'NIT' ? 'selected' : '' }}>NIT</option>
                                    <option value="Otro"
                                        {{ old('id_type', $client->id_type) == 'Otro' ? 'selected' : '' }}>Otro</option>
                                </select>
                            </div>
                            <div class="mb-3">
